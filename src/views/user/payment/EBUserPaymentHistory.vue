<template>
  <EBPageLayout>
    ...
    <EBTable :columns="columns"
      ...
      <template #actions="{ row }">
        <div class="eb-actions">
          ...
          <template v-if="isWithinRefundPeriod(row.paymentTime)">
    ...
    <!-- 分页 -->
    <EBPagination :current-page="currentPage"
                  :page-size="pageSize"
                  :total="total"
                  :disabled="loading"
    ...
    <!-- 退款确认对话框 -->
    <el-dialog v-model="refundDialogVisible"
               title="申请退款"
               width="30%">
        ...
        <el-form ref="refundFormRef"
          ...
          <el-form-item label="退款原因"
            ...
            <el-select v-model="refundForm.reason"
                       placeholder="请选择退款原因"
                       style="width: 100%;">
              <el-option label="重复缴费"
                         value="重复缴费"></el-option>
              <el-option label="缴费金额错误"
                         value="缴费金额错误"></el-option>
              <el-option label="其他原因"
                         value="其他原因"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="refundForm.reason === '其他原因'"
                        label="详细说明"
                        prop="description">
            <el-input v-model="refundForm.description"
                      type="textarea"
                      :rows="3"
                      placeholder="请详细说明退款原因"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <template #footer>
...
const handleDetail = (row) => {
  router.push(`/user/paymentDashboard/detail/${row.paymentId}`);
};
